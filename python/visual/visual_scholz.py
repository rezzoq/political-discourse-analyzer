...
# 1. Загрузка данных
# ----------------------------
df = pd.read_csv("scholz.csv")

# Убираем лишние миллисекунды и пробелы
df['published_at_clean'] = df['published_at'].str.replace(r'\.000$', '', regex=True)

# Конвертируем в datetime
df['published_at_dt'] = pd.to_datetime(df['published_at_clean'], format="%Y-%m-%d %H:%M:%S", errors='coerce')
...
df['intent'] = pd.Categorical(df['intent'], categories=["declarative","imperative","evaluative"], ordered=True)
df['emotion'] = pd.Categorical(df['emotion'], categories=["negative","neutral","positive"], ordered=True)
df['formality'] = pd.Categorical(df['formality'], categories=["informal","formal"], ordered=True)
...
emotion_map = {"negative": "негативная", "neutral": "нейтральная", "positive": "позитивная"}
intent_map = {"declarative": "декларативное", "imperative": "императивное", "evaluative": "оценочное"}
formality_map = {"informal": "неформальная", "formal": "формальная"}
...
# ----------------------------
intent_counts = df.groupby(['year', 'intent_ru']).size().unstack(fill_value=0)
...
plt.tight_layout()
plt.savefig('шольц Коммуникативное намерение.png', dpi=150, bbox_inches='tight')   # для Бербок – 'бербок Коммуникативное намерение.png'

# ----------------------------
# 3. Emotion по годам (Рисунок 2.4)
# ----------------------------
emotion_counts = df.groupby(['year', 'emotion_ru']).size().unstack(fill_value=0)
# фиксируем порядок
...
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('шольц emotion.png', dpi=150, bbox_inches='tight')
...
# plt.show()
# Подсчитываем доли
formality_counts = df.groupby(['year', 'formality_ru']).size().unstack(fill_value=0)
...
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig('шольц Формальность.png', dpi=150, bbox_inches='tight')
...
# ----------------------------
plt.figure()
df_density = df.groupby('year')['value_density'].mean().reset_index()
...
plt.tight_layout()
plt.savefig('шольц плотность ценностной лексики.png', dpi=150, bbox_inches='tight')
...
plt.figure()
df_values = df.groupby('year')[value_cols].mean().reset_index()
...
plt.legend()
plt.tight_layout()
plt.savefig('шольц ключевые ценности.png', dpi=150, bbox_inches='tight')

# ----------------------------
# 7. Rhetorical intensity по годам (Рисунок 2.8)
# ----------------------------
plt.figure()
df_r = df.groupby('year')['r_value'].mean().reset_index()
sns.barplot(data=df_r, x='year', y='r_value', color='crimson')
plt.title("Риторическая насыщенность (R)")
plt.xlabel("Год")
plt.ylabel("Среднее значение R")
plt.tight_layout()
plt.savefig('шольц Риторическая насыщенность (R).png', dpi=150, bbox_inches='tight')
# ----------------------------
# 8. Theatricality (Рисунок 2.9)
# ----------------------------
plt.figure()
df_theat = df.groupby('year')['theatricality'].mean().reset_index()
...
plt.tight_layout()
plt.savefig('шольц театральность.png', dpi=150, bbox_inches='tight')
...
build_series(df_modal, 'modal_strength', 'darkorange',
             ...
             'шольц Интенсивность модальных конструкций.png')
