...
PRONOUNS = ["ich", "du", "er", "sie", "es", "wir", "ihr", "mich", "dich", "ihn", "uns", "euch",
            "mein", "dein", "sein", "ihr", "unser", "euer"]  # базовый набор
THEATRICAL_WORDS = [
    "historisch", "beispiellos", "entscheidend", "dramatisch",
...
IRONY_DE = [
    "offensichtlich", "sicher", "ja genau", "als ob", "definitiv",
    "klar", "zweifellos", "absolut", "perfekt", "wunderbar"
]
def detect_sarcasm_de(text):
    text_lower = text.lower()
    return 1 if any(w in text_lower for w in IRONY_DE) else 0
# ----------------------------
# 3. Загружаем данные
# ----------------------------
...
df["sarcasm_flag"] = df["quote"].apply(detect_sarcasm_de)
# ----------------------------
...
for index, row in df.iterrows():
    ...
    # внутри цикла for index, row in df.iterrows():
    text = row['quote']
    features = detect_rhetoric(text)
    # ... сохранение features ...
...
shutil.copy("labeled_statements_filled_rhetoric.csv", "../visual/baerbok.csv")
